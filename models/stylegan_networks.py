...
class StyleGAN2Decoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect',
        ...
        for i in range(num_downsampling):
            ...
            convs.append(
                StyledConv(in_channel, out_channel, 3, upsample=True, blur_kernel=blur_kernel,
...
class StyleGAN2EnCoGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect',
                 no_antialias=False, opt=None):
        super().__init__()
        self.opt = opt
    ...
    def forward(self, input, layers=[], encode_only=False, CUTv2=False):
        # feat, feats = self.encoder(input, layers, True)
        # if encode_only:
        #     return feats
        # else:
        ...
        feats = []
        # print(feat.shape)
        if CUTv2 and 0 in layers:
            feats.append(feat)
            layers = layers[1:]  # remove 0 from list

        for layer_id, layer in enumerate(chain(self.encoder, self.decoder)):
            # print(layer_id, layer)
            feat = layer(feat)
            # print(layer_id, layer._get_name(), feat.shape)
            if layer_id in layers:
                feats.append(feat)
        ...
        # 0 Identity torch.Size([32, 3, 64, 64])
        # 1 ConvLayer torch.Size([32, 80, 64, 64])
        # 2 ResBlock torch.Size([32, 160, 32, 32])
        # 3 ResBlock torch.Size([32, 160, 32, 32])
        # 4 ResBlock torch.Size([32, 160, 32, 32])
        # 5 ResBlock torch.Size([32, 160, 32, 32])
        # 6 ResBlock torch.Size([32, 160, 32, 32])
        # 7 ResBlock torch.Size([32, 160, 32, 32])
        # 8 ResBlock torch.Size([32, 160, 32, 32])
        # 9 StyledConv torch.Size([32, 80, 64, 64])
        # 10 ConvLayer torch.Size([32, 3, 64, 64])